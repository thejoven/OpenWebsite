...
import { Activity, Database, Gauge, Globe2, Newspaper, RefreshCw, ShieldCheck } from "lucide-react";
...
const modules = [
  ...
  { icon: Newspaper, titleZh: "文章与分类", titleEn: "Articles and categories" },
      ...
      <section className="relative isolate overflow-hidden bg-[#081a30] py-20 text-white md:py-28">
        ...
        <div className="container relative z-10 grid min-h-[560px] content-center">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-6 text-5xl font-black leading-[1.02] md:text-7xl">{t("title")}</h1>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-[#d8e4f2] md:text-xl">{t("subtitle")}</p>
            ...
            <div className="mt-10 grid max-w-2xl gap-3 text-sm font-bold text-[#d8e4f2] sm:grid-cols-3">
              {[t("proofCountries"), t("proofProjects"), t("proofPower")].map((item) => (
      ...
      <section className="bg-white py-16">
        <div className="container">
          <div className="max-w-2xl">
            <h2 className="text-3xl font-black md:text-4xl">{t("productsTitle")}</h2>
            <p className="mt-4 text-lg leading-8 text-[#5a6a7f]">{t("productsSubtitle")}</p>
          </div>
          <div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {modules.map((item) => {
              const Icon = item.icon;
              return (
                <div className="rounded-md border border-[#d9e3ef] bg-[#f7f9fc] p-5" key={item.titleEn}>
                  <Icon aria-hidden className="h-7 w-7 text-[#0e4fa0]" />
                  <h3 className="mt-5 text-xl font-black">{isEn ? item.titleEn : item.titleZh}</h3>
      ...
      <section className="bg-[#edf3f8] py-16">
        <div className="container grid gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
          ...
          <div>
            ...
            <p className="mt-4 text-lg leading-8 text-[#5a6a7f]">{t("whySubtitle")}</p>
            <div className="mt-8 grid gap-3">
              {[
                isEn ? "Search pages output metadata, canonical URLs and hreflang." : "公开页面输出 metadata、canonical 与 hreflang。",
                isEn ? "Admin changes revalidate affected public pages immediately." : "后台保存内容后即时刷新受影响页面。",
                isEn ? "SQLite keeps deployment small and volume-friendly." : "SQLite 降低部署复杂度，并通过数据卷持久化。"
              ].map((item) => (
                <div className="flex gap-3 rounded-md bg-white p-4" key={item}>
                  <ShieldCheck aria-hidden className="mt-0.5 h-5 w-5 shrink-0 text-[#177d78]" />
