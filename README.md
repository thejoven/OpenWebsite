...
- 页面基础：首页、关于我们、产品 / 服务、文章列表、分类落地页、文章详情、联系我们
- SEO：动态 metadata、canonical、hreflang + `x-default`、Open Graph、Twitter Card、自动化 `sitemap.xml`、`robots.txt`、文章 JSON-LD、站点 / 分类 / 文章独立 SEO 字段
...
- 发布：保存文章、分类或站点设置后触发 `revalidatePath`，`sitemap.xml` 按内容和语言自动生成
...
前台公开页面设置了 `revalidate`，并且数据库不可用时会使用内置示例内容回退，保证构建阶段不会因为 SQLite 尚未迁移而失败。实际运行时，后台保存内容后会按 locale 重新验证首页、服务、关于、文章列表、分类落地页、文章详情、联系页、`sitemap.xml` 和 `robots.txt`。

`/sitemap.xml` 由 `src/lib/sitemap.ts` 自动汇总多语言静态页面、分类落地页和已发布文章，输出 hreflang alternates 与 `x-default`，并按小时重新生成。后台「SEO 医生」和「定时任务」页面都可以手动刷新 sitemap，并记录上次刷新结果。
